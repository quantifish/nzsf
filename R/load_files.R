...
system.file("extdata", "nz-coastlines-and-islands-polygons-topo-150k", package = "testdat")

sf_coast <- st_read(dsn = "shapefiles", layer = "nz-coastlines-and-islands-polygons-topo-150k") %>% 
  st_transform(crs = proj) %>%
  rmapshaper::ms_simplify(keep = 0.001, keep_shapes = FALSE)
depth_sf_km <- st_read(dsn = "shapefiles", layer = "depth-contour-polyline-hydro-1350k-11500k") %>%
  filter(VALDCO %in% c(200)) %>%
  rename(depth = VALDCO) %>%
  st_transform(crs = proj, check = TRUE)
sf_qma <- st_read(dsn = "shapefiles", layer = "QMA_Spiny_Red_Rocklobster") %>% st_transform(crs = proj)
sf_stat <- st_read(dsn = "shapefiles", layer = "rock_lobster_stat_areas") %>% st_transform(crs = proj)
