...
plot_qma <- function(proj = "+proj=aea +lat_1=-30 +lat_2=-50 +lat=-40 +lon_0=175 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs", 
                     qma = "CRA", ...) {
  x <- get_qma(qma = qma) %>% st_transform(crs = proj, check = TRUE)
  geom_sf(data = x, ...)
}
