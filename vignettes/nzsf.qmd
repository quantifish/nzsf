...
Layers such as New Zealand marine reserves, depth contours, and Quota Management Areas (QMAs) can be added easily with several of the `nzsf` helper functions including `plot_marine_reserves`, `plot_depth`, and `plot_qma`. Maps can be restricted (e.g. to the North Island only) using a bounding box generated using `st_bbox` from the `sf` package:
